...
import { mkdir as mkdir3, mkdtemp, readFile as readFile3, rm as rm3, writeFile as writeFile3 } from "node:fs/promises";
...
function isManagedSkillInstall(skillRoot = resolveSkillRoot()) {
  return existsSync(path2.join(skillRoot, "SKILL.md"));
}
...
async function runUpdaterHelper(payloadPath) {
  ...
  const helperSource = await readFile3(bundledHelperPath, "utf8");
  await writeFile3(helperCopyPath, helperSource, "utf8");
...
var InstagramInsightsApiClient = class {
  ...
  async requestJson(path4, init, allowRetry = true) {
    const state = await this.requireAuthenticatedState();
    const response = await fetch(`${this.appUrl}${path4}`, {
      ...init,
      headers: {
